...
import { Loader2, FileText, Link as LinkIcon, Sparkles, Share2, Copy, ArrowLeft } from "lucide-react";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      <div className="container mx-auto px-4 py-12 max-w-7xl">
        <div className="flex justify-between items-center mb-12">
          <div>
            <h1 className="text-5xl font-bold text-slate-900 mb-2" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
              Partner Content Hub
            </h1>
            <p className="text-lg text-slate-600">Manage and automate your content workflow</p>
          </div>
          <Button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((project) => (
              <Card
                ...
                onClick={() => navigate(`/workshop/${project.id}`)}
              >
                <CardHeader>
...
const CreateProject = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigate(`/workshop/${response.data.id}`);
    } catch (error) {
